            ...
            <div className="flex flex-wrap gap-4">
              {topics.map((topic: string) => {
                return (
                  <div
                    key={topic}
                    className="group py-2 flex justify-between items-center border-b border-foreground/60 dark:border-primary-foreground/40 hover:text-primary cursor-pointer"
                  >
                    <span>{topic}</span>
                    <ChevronRight className="group-hover:-mr-[5.67px] h-4.5! max-w-0 group-hover:max-w-4.5 -mb-px text-primary opacity-0 group-hover:opacity-100 transition-all duration-300 ease-in-out pointer-events-none group-hover:pointer-events-auto" />
                  </div>
                );
              })}
            </div>
